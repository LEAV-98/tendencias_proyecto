...
function listar_tramite() {
  ...
  tbl_tramite = $("#tabla_tramite").DataTable({
    ...
    columns: [
      ...
      {
        defaultContent:
          "<button class='seguimiento btn btn-success btn-sm'><i class='fa fa-search'></i></button>",
      },
      { data: "origen" },
      { data: "destino" },
      {
...
$("#tabla_tramite").on("click", ".derivar", function () {
  ...
  document.getElementById("lbl_titulo_derivar").innerHTML =
    "<b>DERIVAR O FINALIZAR TRAMITE</b> " + data.documento_id;
...
$("#tabla_tramite").on("click", ".seguimiento", function () {
  ...
  document.getElementById("lbl_titulo").innerHTML =
    "SEGUIMIENTO DEL TRAMITE " + data.documento_id;
...
$("#tabla_tramite").on("click", ".mas", function () {
  ...
  document.getElementById("lbl_titulo_datos").innerHTML =
    "DATOS DEL TRAMITE " + data.documento_id;
  ...
  $("#select_area_d")
    .select2()
    .val(data.area_destino)
    .trigger("change.select2");
  $("#select_tipo")
    .select2()
    .val(data.tipodocumento_id)
    .trigger("change.select2");
  ...
  $.ajax({
    ...
    success: function (resp) {
      if (resp.length > 0) {
        ...
        Swal.fire(
          "Mensaje de Advertencia",
          "No se pudo completar el procedo",
          "warning"
        );
      }
    },
  ...
  $.ajax({
    ...
    success: function (resp) {
      if (resp.length > 0) {
        ...
        Swal.fire(
          "Mensaje de Advertencia",
          "El Usuairo ingresado ya se encuentra en la base de datos",
          "warning"
        );
      }
    },
...
function Buscar_reniec() {
  ...
  if (input.value.length < 8) {
    ...
    return Swal.fire(
      "Mensaje de Advertencia",
      "El campo <b>dni</b>  debe tener como minimo 8 d&iacute;gitos",
      "warning"
    );
  } else {
  ...
  $.ajax({
    ...
    .done(function (resp) {
      if (resp["name"] != undefined) {
        ...
        Swal.fire(
          "Mensaje de Advertencia",
          "<b style='color:#9B0000'>Lo sentimos el dni ingresado no se encuentro en los archivos de la reniec</b>",
          "warning"
        );
      }
    })
    .fail(function (jqXHR, textStatus, errorThrown) {
      ...
      if (jqXHR.status === 401) {
        Swal.fire({
          ...
          html: "<b style='color:#9B0000;font-size:20px'>Acceso no autorizado!!!</b><br><b style='font-size:14px'><b style='color:#9B0000;'>Membres&iacute;a vencida</b>, Para mayor informaci&oacute;n comun&iacute;quese con su proveedor</b>",
          imageUrl:
            "https://softnetpe.com/Sistema_MesaPartes_reniec/Vista/img/reniec.png",
          imageWidth: 120,
          imageHeight: 115,
          imageAlt: "Cargando...",
        });
